	...
	<ul class="pagination .pagination-lg">
		<c:choose>
			<c:when test="${pageInfo.pageNum!=1 }">
				<li><a href="${pageScope.targetUrl}/1" aria-label="Previous"><span aria-hidden="true">&laquo;首页</span></a></li>
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${pageInfo.pageNum!=pageInfo.pages}">
				<li><a href="${pageScope.targetUrl}/${pageInfo.pages }" aria-label="Next"> <span aria-hidden="true">&raquo;末页</span></a></li>
			</c:when>
			<c:otherwise>
				<li class="disabled"><span aria-hidden="true">&raquo;末页</span></li>
			</c:otherwise>
		</c:choose>
	</ul>
</nav>